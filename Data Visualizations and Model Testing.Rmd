...
Considering the distribution of distance - it is perhaps better modeled by a log:
```{r}
table %>% 
  ggplot(aes(x = distance)) + geom_histogram()

table %>% 
  ggplot(aes(x = log(distance))) + geom_histogram()

```

```{r}
table$log <- log(table$distance)
```
...
```{r}
benchmarkPRI.1.log <- lm( EPNa ~ log*PRIstr*highTURNOUT+
                        log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+ 
...
benchmarkPRD.1.log <- lm( AMLOa ~ log*PRIstr*highTURNOUT+
                        log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+     
                         PRI09a+PRD09a+PAN09a+PART09+
                         lnpop+P18+P65+area+density+INDIGENOUS+CATHOLIC+NONRELIGIOUS+
                         EDUCATION+POSTEDUC+ILLITERACY+PROM_HNV+
...
benchmarkPAN.1.log <- lm( JVMa ~ log*PRIstr*highTURNOUT+log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+     
                         PRI09a+PRD09a+PAN09a+PART09+
                         lnpop+P18+P65+area+density+INDIGENOUS+CATHOLIC+NONRELIGIOUS+
                         EDUCATION+POSTEDUC+ILLITERACY+PROM_HNV+
                         PEAprop+PEAfemale+NOINSURANCE+FEMALEJEFA+
...
benchmarkPART.1.log <- lm( PART ~ log*PRIstr*highTURNOUT+log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+     
                          PRI09a+PRD09a+PAN09a+PART09+
                          lnpop+P18+P65+area+density+INDIGENOUS+CATHOLIC+NONRELIGIOUS+
                          EDUCATION+POSTEDUC+ILLITERACY+PROM_HNV+
                          PEAprop+PEAfemale+NOINSURANCE+FEMALEJEFA+
...
knitr::kable(broom::tidy(benchmarkPRI.1.log))
```
...
##Log Distance plot
```{r}
g <- glm(EPNa ~ log*PRIstr*highTURNOUT+log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+     
           PRI09a+PRD09a+PAN09a+PART09+
           lnpop+P18+P65+area+density+INDIGENOUS+CATHOLIC+NONRELIGIOUS+
           EDUCATION+POSTEDUC+ILLITERACY+PROM_HNV+
           PEAprop+PEAfemale+NOINSURANCE+FEMALEJEFA+
...
at.logdist <- seq(-4.5, 4.6, .01)
marg<-marg(mod = g, var_interest = c("log"), type = 'levels', data=table[table$PRDstr==1 & table$highTURNOUT==1,],
           at_var_interest = seq(-4.5, 4.6, .01),  cofint=.95)
...
dataPRI<-data.frame(cbind(at.logdist, mean, upper, lower))
dataPRI$distance<-exp(dataPRI$at.logdist)


#same for AMLO
g <- glm(AMLOa ~ log*PRIstr*highTURNOUT+log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+     
           PRI09a+PRD09a+PAN09a+PART09+
           lnpop+P18+P65+area+density+INDIGENOUS+CATHOLIC+NONRELIGIOUS+
           EDUCATION+POSTEDUC+ILLITERACY+PROM_HNV+
           PEAprop+PEAfemale+NOINSURANCE+FEMALEJEFA+
...
dataPRD<-data.frame(cbind(at.logdist, mean, upper, lower))
dataPRD$distance<-exp(dataPRI$at.logdist)

g <- glm(JVMa ~ log*PRIstr*highTURNOUT+log*PRDstr*highTURNOUT+log*PANstr*highTURNOUT+     
           PRI09a+PRD09a+PAN09a+PART09+
           lnpop+P18+P65+area+density+INDIGENOUS+CATHOLIC+NONRELIGIOUS+
           EDUCATION+POSTEDUC+ILLITERACY+PROM_HNV+
           PEAprop+PEAfemale+NOINSURANCE+FEMALEJEFA+
...
dataPAN<-data.frame(cbind(at.logdist, mean, upper, lower))
dataPAN$distance<-exp(dataPRI$at.logdist)
...
```{r}
datasim$dv<-NA
datasim$dv[1:911]<-"Pena Nieto" # Chengyu: I have to change the name to Pena
datasim$dv[912:1822]<-"López Obrador"
datasim$dv[1823:2733]<-"Vázquez Mota"
```
...
ggplot()+
  ...
  geom_ribbon(data=datasim, aes(x=distance,  ymin=lower, 
                      ymax=upper, fill=dv),  alpha=0.5)+theme_bw()+
  ylab("Predicted vote shares (%)") +
  xlab("Distance to Soriana (km)")+xlim(0,20)+ylim(0,50)+ 
...
```{r}
library(sandwich)
head(coeftest(benchmarkPRI.1, vcov = vcovHC(benchmarkPRI.1,type="HC1")))
head(coeftest(benchmarkPRI.1))

head(coeftest(benchmarkPRI.1.proximity, vcov = vcovHC(benchmarkPRI.1.proximity,type="HC1")))
head(coeftest(benchmarkPRI.1.proximity))
...
```{r}
coeftest(benchmarkPRI.1.proximity, vcov = vcovHC(benchmarkPRI.1.proximity,type="HC1"))
coeftest(benchmarkPRI.1.proximity)

coeftest(benchmarkPRI.1.log, vcov = vcovHC(benchmarkPRI.1.log,type="HC1"))
coeftest(benchmarkPRI.1.log)
